    ...
    def __init__(self, dicom_handler, parent=None):
        ...
        for i, dataset in enumerate(self.dicom_handler.current_datasets):
            try:
                ...
                tile_label.setToolTip(f"Image {i+1}")
                ...
                tile_label.mousePressEvent = lambda event, index=i: self.select_image(index)
...
class DicomFolderHandler:
    ...
    def get_dicom_tags(self, index):
        """
        Get DICOM tags for a specific file with human-readable names
        
        Args:
            index (int): Index of the DICOM file
        
        Returns:
            dict: Dictionary of DICOM tags with names and values
        """
        if 0 <= index < len(self.current_datasets):
            ...
            for elem in dataset:
                ...
                try:
                    # First try to get name from pydicom dictionary
                    tag_name = datadict.keyword_for_tag(tag_hex)
                    
                    # If no keyword found, use string representation of tag
                    if not tag_name:
                        tag_name = str(tag_hex)
                except:
                    tag_name = str(tag_hex)
                
                # If tag name is empty, use tag hex
                if not tag_name:
                    tag_name = str(tag_hex)
...
class DicomFolderViewer(QMainWindow):
    ...
    def next_image_cine(self):
        ...
        if self.dicom_handler.dicom_files:
            ...
            next_value = (current_value + 1) % len(self.dicom_handler.dicom_files)
