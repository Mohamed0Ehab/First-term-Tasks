...
class FootballTrackerGUI:
    ...
    def setup_gui(self):
        ...
        # Player selection dropdown (now just for heatmap viewing)
        ...
        self.player_dropdown.set("Select Player for Heatmap")
    ...
    def show_heatmap_window(self):
        ...
        if not selected or selected == "Select Player for Heatmap":
    ...
    def track_video(self):
        ...
        for player in detected_players:
            ...
            if player_id is not None:
                # Draw bounding box for all players
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), 
                            (int(bbox[2]), int(bbox[3])), (255, 0, 0), 2)
                cv2.putText(frame, f'ID: {player_id}', 
                          (int(bbox[0]), int(bbox[1]) - 10),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                current_bboxes.append(player_id)
                
                # Track position for all players
