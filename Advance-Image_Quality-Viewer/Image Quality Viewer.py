...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                           ...
                           QGroupBox, QMessageBox, QShortcut, QStatusBar, QSlider)
...
class ImageViewer(QMainWindow):
    ...
    def initUI(self):
        ...
        # Zoom controls group
        zoom_group = QGroupBox("Resolution Controls")
        ...
        self.zoom_label = QLabel('Resolution: 100%')
        self.zoom_factor.valueChanged.connect(
            lambda v: self.zoom_label.setText(f'Resolution {v}%'))
        ...
        self.crop_to_output1_btn = QPushButton('Crop to Output 1')
        self.crop_to_output1_btn.setToolTip('Crop the image to the selected ROI and send to Output 1')
        self.crop_to_output1_btn.clicked.connect(lambda: self.crop_image_to_output(1))
        file_controls.addWidget(self.crop_to_output1_btn)

        self.crop_to_output2_btn = QPushButton('Crop to Output 2')
        self.crop_to_output2_btn.setToolTip('Crop the image to the selected ROI and send to Output 2')
        self.crop_to_output2_btn.clicked.connect(lambda: self.crop_image_to_output(2))
        file_controls.addWidget(self.crop_to_output2_btn)
    ...
    def save_image(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(self, "Save Image", "", 
                                                 "Image Files (*.png *.jpg *.bmp *.tif *.jpeg)")
        if file_name:
    ...
    def measure_snr_cnr(self):
        ...
        self.statusBar.showMessage(f'Select the noise point then signals points {view_name}', 5000)
        ...
        try:
            ...
            # Calculate metrics
            signal_mean = np.mean(roi2_data)
            noise_mean = np.mean(roi1_data)
            tissue_mean = np.std(roi3_data)
            
            snr = signal_mean / noise_mean if noise_mean != 0 else 0
            cnr = abs(signal_mean - tissue_mean) / noise_mean if noise_mean != 0 else 0
    ...
    def show_histogram(self):
        try:
            ...
            if len(image.shape) == 3:  # Color image
                ...
                for idx, (channel, color) in enumerate(zip(cv2.split(image), ['b', 'g', 'r'])):
                    plt.subplot(2, 2, idx+1)
                    plt.title(f'{color_channels[idx]} Channel')
                ...
                for channel, color in zip(cv2.split(image), ['b', 'g', 'r']):
                    plt.hist(channel.ravel(), 256, [0, 256], color=color, alpha=0.5)
                plt.xlabel('Pixel Value')
                plt.ylabel('Frequency')
                plt.grid(True, alpha=0.3)
    ...
    def crop_image_to_output(self, output_num):
        # Determine which view is currently focused
        if self.input_view.hasFocus():
            current_view = self.input_view
            current_image = self.input_image
            view_name = "Input"
        elif self.output1_view.hasFocus():
            ...
            current_image = self.output1_image
            view_name = "Output 1"
        elif self.output2_view.hasFocus():
            current_view = self.output2_view
            current_image = self.output2_image
            view_name = "Output 2"
        else:
            self.statusBar.showMessage('Please focus on a view before cropping!', 3000)
            return

        if current_image is None:
            self.statusBar.showMessage(f'No image in {view_name} view!', 3000)
            return

        if not current_view.rois:
            self.statusBar.showMessage('No ROI selected for cropping!', 3000)
            return

        try:
            # Use the last ROI for cropping
            roi = current_view.rois[-1]
            scene_rect = current_view.scene().sceneRect()
            ...
            roi_x = int(roi.x() * scale_x)
            roi_y = int(roi.y() * scale_y)
            roi_width = int(roi.width() * scale_x)
            roi_height = int(roi.height() * scale_y)

            # Ensure ROI is within image bounds
            roi_x = max(0, min(roi_x, current_image.shape[1] - 1))
            roi_y = max(0, min(roi_y, current_image.shape[0] - 1))
            roi_width = min(roi_width, current_image.shape[1] - roi_x)
            roi_height = min(roi_height, current_image.shape[0] - roi_y)

            # Crop the image
            cropped_image = current_image[roi_y:roi_y + roi_height, roi_x:roi_x + roi_width]

            # Place the cropped image in the specified output view
            if output_num == 1:
                self.output1_image = cropped_image
                self.display_image(self.output1_image, self.output1_view)
                self.statusBar.showMessage('Cropped image placed in Output 1', 3000)
            elif output_num == 2:
                self.output2_image = cropped_image
                self.display_image(self.output2_image, self.output2_view)
                self.statusBar.showMessage('Cropped image placed in Output 2', 3000)
            else:
                self.statusBar.showMessage('Invalid output number!', 3000)

        except Exception as e:
            self.statusBar.showMessage(f'Error cropping image: {str(e)}', 3000)
            print(f"Error details: {str(e)}")  # For debugging
